...
# Data for US Indices (May 20, 2026 close)
indices = ['S&P 500', 'Nasdaq', 'Dow Jones']
prices = [7432.97, 26270.36, 50009.35]
changes = [1.08, 1.55, 1.31]

fig, ax = plt.subplots(figsize=(10, 6), facecolor='#f0f0f0')
ax.set_facecolor('#ffffff')

# Red for Up in Chinese standard
...
for bar, change, price in zip(bars, changes, prices):
    ...
    ax.text(bar.get_x() + bar.get_width()/2, yval + 0.05 if yval > 0 else yval - 0.05, 
            f'+{change}%' if change > 0 else f'{change}%', ha='center', va='bottom' if yval > 0 else 'top', 
...
ax.set_title('美股核心指数表现 (2026-05-20)', fontproperties=prop, fontsize=18, fontweight='bold', pad=20)
...
plt.savefig(f'{output_dir}/2026-05-21-morning.png', dpi=300)
print(f"Chart saved to {output_dir}/2026-05-21-morning.png")
