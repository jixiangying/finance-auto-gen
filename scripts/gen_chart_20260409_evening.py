...
# Data for April 9, 2026 (Evening)
indices = ['上证指数', '深证成指', '创业板指', '恒生指数', '恒生科技']
values = [3966.17, 13996.27, 3323.30, 25752.40, 4679.10]
changes = [-0.72, -0.33, -0.73, -0.54, -2.06]
...
for i, (bar, change, val) in enumerate(zip(bars, changes, values)):
    ...
    ax.text(label_x, bar.get_y() + bar.get_height()/2, 
            ...
            va='center', ha='right', fontproperties=prop, fontsize=12, fontweight='bold')

# Styling
ax.set_title('2026年04月09日 国内及港股核心指数表现 (收盘)', fontproperties=prop, fontsize=16, pad=20)
ax.set_xlabel('涨跌幅 (%)', fontproperties=prop, fontsize=12)
ax.set_xlim(min(changes)-0.5, 0.5)
...
output_path = os.path.join(output_dir, '2026-04-09-evening.png')
plt.tight_layout()
