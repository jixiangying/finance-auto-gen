...
# Data for April 10, 2026 (Evening)
indices = ['上证指数', '深证成指', '创业板指', '恒生指数', '恒生科技']
values = [3986.22, 14309.47, 3448.79, 25894.04, 4716.53]
changes = [0.51, 2.24, 3.78, 0.55, 0.80]
...
for i, (bar, change, val) in enumerate(zip(bars, changes, values)):
    ...
    ax.text(label_x, bar.get_y() + bar.get_height()/2, 
            ...
            va='center', ha='left', fontproperties=prop, fontsize=12, fontweight='bold')

# Styling
ax.set_title('2026年04月10日 国内及港股核心指数表现 (收盘)', fontproperties=prop, fontsize=16, pad=20)
ax.set_xlabel('涨跌幅 (%)', fontproperties=prop, fontsize=12)
ax.set_xlim(0, max(changes) + 1.0)
...
output_path = os.path.join(output_dir, '2026-04-10-evening.png')
plt.tight_layout()
